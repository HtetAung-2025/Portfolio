...
const Skills = () => {
    const skillCategories = {
        ...
        'Tools & Others': [
            ...
            skills.find(s=>s.name === 'Photoshop'),
            skills.find(s=>s.name === 'Ai Illustrator'),
            skills.find(s=>s.name === 'Microsoft'),
            skills.find(s=>s.name === 'MySQl')
        ].filter(Boolean),
        'Languages Skills':[
            skills.find(s=>s.name === 'Myanmar'),
            skills.find(s=>s.name === 'Japanese'),
            skills.find(s=>s.name === 'English'),
            skills.find(s=>s.name === 'Chinese')
        ].filter(Boolean),
    ...
    const getLevelColor = (level) => {
        const colors = {
            'Native': 'text-[#8DFF69] bg-[#8DFF69]/20 border-[#8DFF69]/30',
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <FadeIn delay={100}>
                    <div className="text-center mb-16">
                        ...
                        <p className="text-lg text-white/60 max-w-2xl mx-auto">プログランミングスキル、言語スキル、その他スキル</p>
                    </div>
                </FadeIn>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {Object.entries(skillCategories).map(([category, categorySkills], categoryIndex) => (
                        <FadeIn key={category} delay={categoryIndex * 100}>
                            <div className="relative bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-primary/30 transition-all duration-500 group">
                                                ...
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                                        <div className="p-2 bg-white/5 rounded-lg">
                                                            <IconComponent className="w-4 y-4 text-primary"/>
                                                        </div>
