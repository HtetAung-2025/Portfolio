...
import { Mail, MapPin, Github, Send, MessageSquare, Icon } from 'lucide-react'
...
const Contact = () => {
    ...
    const handleSubmit = (e) => {
        ...
        console.log('Form submitted', formData); // ← ADD: Debug log
        ...
        console.log('Sending email...'); // ← ADD: Debug log
        ...
        emailjs.send(
            ...
            .then((response) => {
                console.log('SUCCESS!', response); // ← ADD: Check console
                setStatus({
                    type: 'success',
                    message: 'Message sent successfully! I\'ll get back to you soon'
                })
                setFormData({ name: '', email: '', message: '' })
                setTimeout(() => setStatus({ type: '', message: '' }), 5000) // ← MOVED HERE
            })
            .catch((error) => {
                console.error('FAILED!', error); // ← ADD: Check console
                setStatus({
                    type: 'error',
                ...
                <FadeIn delay={0}>
                    <div className="text-center mb-16">
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 border border-primary/30 rounded-full mb-6">
                            ...
                            <span className="text-sm text-primary font-medium tracking-wilder uppercase">Get In Touch</span>
                        </div>
                        <h2 className="text-4xl lg:text-5xl font-normal text-white mb-4">
                            Let's Work Together
                        </h2>
                        <p className="text-lg text-white/60 max-w-2xl mx-auto">
                            プロジェクトのご相談や採用に関するお問い合わせは、以下のフォームよりお気軽にご連絡ください。
                            追って担当よりご返信いたします。
                        </p>
                    </div>
                </FadeIn>
                    ...
                    <FadeIn delay={100}>
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-8">
                            <form
                                ...
                                <div>
                                    <label
                                        htmlFor="email" className="block text-sm font-medium text-white/80 mb-2">
                                        Email
                                    </label>
                                    <input
                                        type="email"
                        ...
                        <div className="space-y-8">
                            <div>
                                ...
                                <p className="text-white/60 leading-relaxed">
                                    I'm always opening discussing new projects, creative ideas
                                </p>
                            </div>
                            <div className="space-y-4">
                                <div className="group relative bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-primary/30 transition-all duration-300">
                                    ...
                                    <div className="absolute inst-0 bg-linear-to-br from-primary/0 to-primary/0 group-hover:from-primary/5 group-hover:to-primary/5 rounded-2xl transition-all duration-300 pointer-events-none" />
                                </div>
                                <div className="group relative bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-primary/30 transition-all duration-300">
                                    <div className="flex items-start gap-4">
                                        <div className="p-3 bg-linear-to-br from-primary/20 to-primary/20 border border-primary/30 rounded-3xl">
